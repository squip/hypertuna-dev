...
import b4a from 'b4a'
import { setTimeout as delay, clearTimeout as clearDelay } from 'bare-timers'
...
async function acquireRemoteTopic (remoteDrive, ttlMs = 90000) {
  ...
  if (entry) {
    ...
    entry.lastUsed = now
    if (entry.timer) {
      try { clearDelay(entry.timer) } catch (_) {}
      entry.timer = null
    }
...
function setTimeoutEvict (key, ttlMs) {
  // Use bare-timers' promise-based delay to schedule eviction without blocking
  const token = {}
  ;(async () => {
    try {
      await delay(ttlMs)
