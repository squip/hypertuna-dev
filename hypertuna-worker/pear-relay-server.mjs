  ...
  if (result.success) {
    ...
    // Auto-authorize the creator
    {
      try {
        ...
        // Use the auth token generated by the relay manager so the
        // profile, auth store and published event all share the same token.
        ...
        // Ensure the in-memory profile reflects the saved auth token
        if (updatedProfile) result.profile = updatedProfile;
